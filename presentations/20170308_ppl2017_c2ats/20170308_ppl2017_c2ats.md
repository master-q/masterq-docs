...
# Rule: Accessor for pointer members #1

```c
// If you have following C structure:
struct t1 {int a;};
struct t2 {struct t1 *tp;};
```

```ats
// c2ats generate following ATS definition:
typedef struct_c2ats_t1 = $extype_struct"struct t1" of {
  a = int
}
typedef struct_c2ats_t2 = $extype_struct"struct t2" of {
  tp = ptr (* cPtr0(struct_c2ats_t1) *)
}
```

# Rule: Accessor for pointer members #2

```ats
// Also, c2ats generate following accessor:
%{#
#ifndef _STRUCT_C2ATS_ACCESSOR_H_
#define _STRUCT_C2ATS_ACCESSOR_H_
static inline struct t1* take_struct_c2ats_t2_tp(struct t2 *p) { return (struct t1*) p->tp; }
#endif /* _STRUCT_C2ATS_ACCESSOR_H_ */
%}
fun take_struct_c2ats_t2_tp: {l1:agz} (!ptr_v_1(struct_c2ats_t2, l1) | ptr l1) -> [l2:addr] (ptr_v_1(struct_c2ats_t1, l2), ptr_v_1(struct_c2ats_t1, l2) -<lin,prf> void | ptr l2) = "mac#"r
```

# Rule: Accessor for pointer members #3
...
* So, we can shape dis-compiler for ATS language?

xxx background figure
