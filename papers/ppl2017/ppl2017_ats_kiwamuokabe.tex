...
ATS言語の関数定義には証明の変数と動的な変数があり、前者はコンパイル時でのみの検査のために使われ、後者はコンパイル後の実コードにそのまま使われます。同様に関数宣言にも証明の引数/動的な引数/証明の返り値/動的な返り値を取ることができます。例えばリスト\ref{proof_dynamic}のキーワード \verb|fun| で定義された関数は \verb|fun_a| という名前がついていています。この関数は \verb|PF1| と \verb|PF2| という型の証明引数と、\verb|char| 型の実引数を取り、\verb|PF3| という型の証明返り値と \verb|int| 型の実返り値を返します。

\begin{lstlisting}[caption=証明の引数と動的な引数,label=proof_dynamic]
fun fun_a: (PF1, PF2 | char) -> (PF3 | int)
\end{lstlisting}

実引数と実返り値のみを取り、証明の値を扱わない関数も作ることができ、さらに \verb|prfun| や \verb|praxi| キーワードを使うことで証明引数だけを取って証明返り値だけを返す関数を定義することもできます。
...
上記のmain.datsはコンソールからリスト\ref{compile-main.dats}のようにATSコンパイラでコンパイルした後実行することができます。
