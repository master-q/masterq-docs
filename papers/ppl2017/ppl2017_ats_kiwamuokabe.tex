...
静的な変数 \verb|n| と \verb|m| を全称量化で導入し、その値は0以上です。\verb|fun_c2ats_fread| 関数の第2実引数と第3実引数はそれぞれ \verb|n| と \verb|m| に依存し、さらに第1実引数である \verb|strnptr| 型の文字列は当該関数の呼び出し前は \verb|n*m| の長さです。
...
\section{謝辞}
C言語の型表現をいかにしてATS言語の型表現に解釈しなおすか、筆者のその疑問に対し根気強いサポートをしてくれたHongwei Xiに感謝します。
