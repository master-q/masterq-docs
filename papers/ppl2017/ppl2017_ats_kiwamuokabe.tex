...
より現実的なアプリケーションとして「自分自身のソースコードを標準出力に印字するアプリケーション」を考えます。やはり以下のようなexample.hファイルがあるとします。
...
これら3つの関数 \verb|fun_c2ats_fopen|, \verb|fun_c2ats_fread|, \verb|fun_c2ats_fclose| を使うことで、このアプリケーションを以下のように構築することができます。

\lstinputlisting{example/fileread/main.dats}

このmain.datsファイルは自動生成したexample.satsファイルと共にコンパイルできますが、以下の点で危険なコードです。
...
\item 危険性4: 任意の型を任意の型にキャストできる \verb|cast| という関数を使っている
\item 危険性5: \verb|my_fread| 関数の返り値は静的な変数 \verb|m| に依存しているがその値の範囲は \verb|m <= n| であるべき
\end{itemize}

次の章では上記の危険性を取り除くことで、段階的に安全なATSコードが手に入る様子を観察します。

\section{現実的なアプリケーション: 段階的な線形型の導入}
\section{現実的なアプリケーション: 安全になったシグニチャ}
